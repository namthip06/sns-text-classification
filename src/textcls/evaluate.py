...
def compute_report(true: np.ndarray, pred: np.ndarray, id2label: dict[int, str]) -> dict:
    """เมตริกบนคลาสที่ปรากฏใน true (คลาสที่ไม่มีข้อมูล → ข้าม ไม่ฉุด macro-F1)."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    report["low_confidence"] = int((probs.max(axis=1) < calib["threshold"]).sum())
    ...
    print(f"accuracy={report['accuracy']:.4f}  macro_f1={report['macro_f1']:.4f}  "
          f"n={report['n_test']:,}  low_conf (<{calib['threshold']})={report['low_confidence']:,}")
    print("per-class:")
